...
/** GuillotineBinPack implements different variants of bin packer algorithms
   that use the GUILLOTINE data structure
        to keep track of the free space of the bin where rectangles may be
   placed. */
...
public:
  ...
  /// whenever you need to restart with a new bin.
  ...
  /// which of the free subrectangles to place the to-be-packed rectangle into.
  ...
  /// Specifies the different choice heuristics that can be used when the packer
  /// needs to decide whether to subdivide the remaining free space in
  /// horizontal or vertical direction.
  enum GuillotineSplitHeuristic {
    SplitShorterLeftoverAxis, ///< -SLAS
    SplitLongerLeftoverAxis,  ///< -LLAS
    SplitMinimizeArea, ///< -MINAS, Try to make a single big rectangle at the
                       ///< expense of making the other small.
    SplitMaximizeArea, ///< -MAXAS, Try to make both remaining rectangles as
                       ///< even-sized as possible.
  ...
  ///		tries to defragment the list of disjoint free rectangles to
  ...
  Rect Insert(int width, int height, bool merge,
              FreeRectChoiceHeuristic rectChoice,
              GuillotineSplitHeuristic splitMethod);

  /// Inserts a list of rectangles into the bin.
  /// @param rects The list of rectangles to add. This list will be destroyed in
  /// the packing process.
  ...
  //	void InsertMaxFitting(std::vector<RectSize> &rects, std::vector<Rect>
  //&dst, bool merge, 		FreeRectChoiceHeuristic rectChoice,
  // GuillotineSplitHeuristic splitMethod);

  /// Computes the ratio of used/total surface area. 0.00 means no space is yet
  /// used, 1.00 means the whole bin is used.
  float Occupancy() const;

  /// Returns the internal list of disjoint rectangles that track the free area
  /// of the bin. You may alter this vector any way desired, as long as the end
  /// result still is a list of disjoint rectangles.
  std::vector<Rect> &GetFreeRectangles() { return freeRectangles; }

  /// Returns the list of packed rectangles. You may alter this vector at will,
  /// for example, you can move a Rect from this list to the Free Rectangles
  /// list to free up space on-the-fly, but notice that this causes
  /// fragmentation.
  ...
  /// free rectangles and merges them if they can be represented with a single
  /// rectangle. Takes up Theta(|freeRectangles|^2) time.
  void MergeFreeList();
...
private:
  ...
  /// used only to compute the Occupancy ratio, so if you want to have the
  /// packer consume less memory, this can be removed.
  std::vector<Rect> usedRectangles;

  /// Stores a list of rectangles that represents the free area of the bin. This
  /// rectangles in this list are disjoint.
  ...
  /// Goes through the list of free rectangles and finds the best one to place a
  /// rectangle of given size into. Running time is Theta(|freeRectangles|).
  ...
  static int ScoreByHeuristic(int width, int height, const Rect &freeRect,
                              FreeRectChoiceHeuristic rectChoice);
  // The following functions compute (penalty) score values if a rect of the
  // given size was placed into the given free rectangle. In these score values,
  // smaller is better.
  ...
  static int ScoreWorstShortSideFit(int width, int height,
                                    const Rect &freeRect);
