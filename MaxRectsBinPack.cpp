/** @file MaxRectsBinPack.cpp
        ...
        @brief Implements different bin packer algorithms that use the MAXRECTS
   data structure.
...
Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height,
  ...
  for (size_t i = 0; i < freeRectangles.size(); ++i) {
    ...
    if (binAllowFlip && freeRectangles[i].width >= height &&
...
void MaxRectsBinPack::PruneFreeList() {
  ...
  through each pair.
  ///  But unfortunately it doesn't quite cut it, since we also want to detect
  containment.
