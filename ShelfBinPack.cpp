/** @file ShelfBinPack.cpp
        ...
        @brief Implements different bin packer algorithms that use the SHELF
   data structure.
...
bool ShelfBinPack::FitsOnShelf(const Shelf &shelf, int width, int height,
...
void ShelfBinPack::RotateToShelf(const Shelf &shelf, int &width,
                                 int &height) const {
  // If the width > height and the long edge of the new rectangle fits
  // vertically onto the current shelf, flip it. If the short edge is larger
  // than the current shelf height, store the short edge vertically.
  if ((width > height && width > binWidth - shelf.currentX) ||
      (width > height && width < shelf.height) ||
      (width < height && height > shelf.height &&
       height <= binWidth - shelf.currentX))
    swap(width, height);
}

void ShelfBinPack::AddToShelf(Shelf &shelf, int width, int height,
                              Rect &newNode) {
...
Rect ShelfBinPack::Insert(int width, int height, ShelfChoiceHeuristic method) {
  ...
  case ShelfBestAreaFit: {
    ...
    for (size_t i = 0; i < shelves.size(); ++i) {
      // Pre-rotate the rect onto the shelf here already so that the area fit
      // computation is done correctly.
      RotateToShelf(shelves[i], width, height);
      if (FitsOnShelf(shelves[i], width, height, i == shelves.size() - 1)) {
        unsigned long surfaceArea =
            (binWidth - shelves[i].currentX) * shelves[i].height;
  ...
  case ShelfBestHeightFit: {
    ...
    for (size_t i = 0; i < shelves.size(); ++i) {
      // Pre-rotate the rect onto the shelf here already so that the height fit
      // computation is done correctly.
      RotateToShelf(shelves[i], width, height);
      if (FitsOnShelf(shelves[i], width, height, i == shelves.size() - 1)) {
  ...
  case ShelfBestWidthFit: {
    // Best Width Fit rule: Choose the shelf with smallest remaining shelf
    // width.
    Shelf *bestShelf = 0;
  ...
  case ShelfWorstWidthFit: {
    // Worst Width Fit rule: Choose the shelf with smallest remaining shelf
    // width.
    Shelf *bestShelf = 0;
  ...
  /*
          ///\todo This is problematic: If we couldn't start a new shelf -
     should we give up
          ///      and move all the remaining space of the bin for the waste map
     to track,
          ///      or should we just wait if the next rectangle would fit
     better? For now,
          ///      don't add the leftover space to the waste map.
          else if (useWasteMap)
          {
                  assert(binHeight - shelves.back().startY >=
     shelves.back().height); shelves.back().height = binHeight -
     shelves.back().startY; if (shelves.back().height > 0)
                          MoveShelfToWasteMap(shelves.back());

                  // Try to pack the rectangle again to the waste map.
                  GuillotineBinPack::Node node = wasteMap.Insert(width, height,
     true, 1, 3); if (node.height != 0)
                  {
                          newNode.x = node.x;
                          newNode.y = node.y;
...
void ShelfBinPack::MoveShelfToWasteMap(Shelf &shelf) {
  ...
  // Add the space after the shelf end (right side of the last rect) and the
  // shelf right side.
  Rect newNode;
  newNode.x = shelf.currentX;
  newNode.y = shelf.startY;
